...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, ExtCtrls, Math,
  Buttons, StdCtrls, Printers, TntGraphics, TntClasses, TypInfo, Clipbrd, ComObj, pBarcode1D, pCore2D,
  pBarcode2D, pQRCode, pCode128, pPDF417, pDataMatrix, pDataMatrixEcc200, pCore1D, jpeg, pCode39;
...
type
  ...
  private
    ...
    PubHS128Cnt : integer;
    PubHS39Code : array[1..BarCodeCnt] of TBarcode1D_Code39;
    PubHS39Cnt : integer;
...
begin
    ...
    for i := 1 to BarCodeCnt do begin
        ...
        if Assigned(PubHS39Code[i]) then FreeAndNil(PubHS39Code[i]);
...
var
    ...
    MyFontColor : TColor;
begin
    if Copy(FontNM,1,1) = '$' then begin // 2022.02.24 BOX그리는것과는 다르게 $가 붙으면 흰색글씨이며 $BOX를 먼저 그린 후 TXT를 써야 함.. $BOX 와 $폰트명은 짝을 이뤄야 함
        Delete(FontNM,1,1);
        MyFontName := FontNM;
        MyFontColor := clWhite;
    end else begin
    ...
    Canvas.Font.Color := MyFontColor; // 2022.02.24
...
var
    ...
    pRect : TRect;
    ImageName : string;
    bAllowEscape : boolean;
    iRatio : double;
    MyIF : string;
    PDF417RowHeight : integer;
begin
    PubHSQRCnt := 1;
    PubHS128Cnt := 1;
    PubHS39Cnt := 1;
  ...
  try
    ...
    for i := 0 to MapList.Count -1 do begin
        ...
        end else if UpperCase(Trim(PositionList.Strings[0])) = 'IFTXT' then begin
            Txt := UpperCase(Trim(PositionList.Strings[7]));
            MyIF := Copy(UpperCase(Trim(PositionList.Strings[9])),1,1);
            if DataList.Count > DataCnt then begin
                if Copy(DataList.Strings[DataCnt],1,1) <> MyIF then continue;
            end;
...
//                    MyTxt := '@FIELD';
        ...
        end else if UpperCase(Trim(PositionList.Strings[0])) = 'IMG' then begin
            FOldMap := SetMapMode(MyCanvas.Handle,MM_LOMETRIC);
            ImageName := ExtractFilePath(Application.ExeName) + 'Image\' + Trim(PositionList.Strings[5]);
            if not FileExists(ImageName) then continue;
            Image := TImage.Create(nil);
            Image.Picture.LoadFromFile(ImageName);
            pRect.Left := StrToInt(Trim(PositionList.Strings[1]))+xLeft;
            pRect.Top := StrToInt(Trim(PositionList.Strings[2]))-xTop;
            pRect.Right := StrToInt(Trim(PositionList.Strings[3]))+xLeft;
            pRect.Bottom := StrToInt(Trim(PositionList.Strings[4]))-xTop;
            MyCanvas.StretchDraw(pRect,Image.Picture.Graphic);
            FreeAndNil(Image);
            ...
            SetMapMode(MyCanvas.Handle,FOldMap);
        end else if UpperCase(Trim(PositionList.Strings[0])) = '$BOX' then begin // 2022.02.24 검정색 박스
            FOldMap := SetMapMode(MyCanvas.Handle,MM_LOMETRIC);
            ...
            SetMapMode(MyCanvas.Handle,FOldMap);
        end else if UpperCase(Trim(PositionList.Strings[0])) = 'IFLINE' then begin // 라인을 조건에 따라 그리거나 그리지 않게 하기 위해....
            if DataList.Count > DataCnt then begin
                MyTxt := UpperCase(DataList.Strings[DataCnt]);
            end else begin
                MyTxt := 'Y';
            end;
            Inc(DataCnt);
            if MyTxt = 'N' then continue;
            ...
            if Length(Trim(MyTxt)) = 0 then continue;
            if pOutput = nil then begin
            ...
            if MyTxt = '@FIELD' then continue;
            if Length(Trim(MyTxt)) = 0 then continue;
            if PositionList.Count > 6 then begin
                PDF417RowHeight := StrToIntDef(Trim(PositionList.Strings[6]),3);
            end else begin
                PDF417RowHeight := 3;
            end;
            if pOutput = nil then begin
                ...
                HSPDF417.RowHeight := PDF417RowHeight;
                ...
                PubHSPDF417[PubHSPDF417Cnt].RowHeight := PDF417RowHeight;
            ...
            if MyTxt = '@FIELD' then continue;
            if Length(Trim(MyTxt)) = 0 then continue;
            if PositionList.Count > 7 then bAllowEscape := StrToBoolDef(Trim(PositionList.Strings[6]),False);
            if pOutput = nil then begin
                GetMargin(xMargin,yMargin);
                ...
                if PositionList.Count > 7 then HSDataMatrixEcc200.AllowEscape := bAllowEscape;
                ...
                if PositionList.Count > 7 then PubHSDataMatrixEcc200[PubHSDataMatrixEcc200Cnt].AllowEscape := bAllowEscape;
...
//                PubHS128Code[PubHS128Cnt].AutoCheckDigit := False;
        ...
        end else if UpperCase(Trim(PositionList.Strings[0])) = '1D39' then begin
            if UpperCase(Trim(PositionList.Strings[7])) = '@FIELD' then begin
                if DataList.Count > DataCnt then begin
                    MyTxt := DataList.Strings[DataCnt];
                end else begin
            ...
            iRatio := 0.0;
            if PositionList.Count > 10 then begin
                iRatio := StrToFloatDef(Trim(PositionList.Strings[11]),2.0);
            end;
            if pOutput = nil then begin
                GetMargin(xMargin,yMargin);

                HS39Code := TBarcode1D_Code39.Create(nil);
                HS39Code.Barcode := MyTxt;
                HS39Code.AutoCheckDigit := False;
                if iRatio > 0.0 then begin
                    HS39Code.Ratio := iRatio;
                end;
                if DisPlayCheck then begin
                    HS39Code.DisplayText := dtBarcode;
                    HS39Code.TextPosition := tpBottomOut;
                    HS39Code.TextAlignment := taCenter;
                    HS39Code.TextFont.Size := DisplayFontSize;
                    HS39Code.TextFont.Name := DisplayFontName;
                    HS39Code.TextCSpacing := 0;
                    HS39Code.TextVSpacing := 1;
                end else begin
                    HS39Code.DisplayText := dtNone;
                end;
                HS39Code.Print(StrToInt(Trim(PositionList.Strings[1]))+xMargin,StrToInt(Trim(PositionList.Strings[2]))-yMargin,StrToFloat(Trim(PositionList.Strings[3])),StrToInt(Trim(PositionList.Strings[4])),StrToInt(Trim(PositionList.Strings[5])),StrToInt(Trim(PositionList.Strings[6])));
                FreeAndNil(HS39Code);
            end else begin
                PubHS39Code[PubHS39Cnt] := TBarcode1D_Code39.Create(nil);
                PubHS39Code[PubHS39Cnt].Barcode := MyTxt;
                PubHS39Code[PubHS39Cnt].Height := Round(StrToInt(Trim(PositionList.Strings[5])) * 3.779);
                PubHS39Code[PubHS39Cnt].LeftMargin := Round(StrToInt(Trim(PositionList.Strings[1])) * 3.779); // 픽셀을 mm로
                PubHS39Code[PubHS39Cnt].TopMargin := Round(StrToInt(Trim(PositionList.Strings[2])) * 3.779);
                PubHS39Code[PubHS39Cnt].AutoCheckDigit := False;
                if iRatio > 0.0 then begin
                    PubHS39Code[PubHS39Cnt].Ratio := iRatio;
                end;

                if DisPlayCheck then begin
                    PubHS39Code[PubHS39Cnt].DisplayText := dtBarcode;
                    PubHS39Code[PubHS39Cnt].TextPosition := tpBottomOut;
                    PubHS39Code[PubHS39Cnt].TextAlignment := taCenter;
                    PubHS39Code[PubHS39Cnt].TextFont.Size := DisplayFontSize;
                    PubHS39Code[PubHS39Cnt].TextFont.Name := DisplayFontName;
                    PubHS39Code[PubHS39Cnt].TextCSpacing := 0;
                    PubHS39Code[PubHS39Cnt].TextVSpacing := 1;
                end else begin
                    PubHS39Code[PubHS39Cnt].DisplayText := dtNone;
                end;

                if StrToFloat(Trim(PositionList.Strings[3])) * 3.779 <= 1.7 then begin
                    PubHS39Code[PubHS39Cnt].Module := 1;
                end else if (StrToFloat(Trim(PositionList.Strings[3])) * 3.779 > 1.7) and (StrToFloat(Trim(PositionList.Strings[3])) * 3.779 <= 2.4) then begin
                    PubHS39Code[PubHS39Cnt].Module := 2;
                end else if (StrToFloat(Trim(PositionList.Strings[3])) * 3.779 > 2.4) then begin
                    PubHS39Code[PubHS39Cnt].Module := 3;
                end;
                PubHS39Code[PubHS39Cnt].Image := pOutput;
                PubHS39Code[PubHS39Cnt].Draw;
